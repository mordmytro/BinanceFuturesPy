...
class MarketData:
    def __init__(
        self, testnet: bool = False, symbol: str = "btcusdt", interval: str = "1m"
    ):
    ...
    def historical_trades(self, fromId: int = None, limit: int = 500):
        """
        To change limit -> limit = 1000
        (max 1000)
        """
        return requests.get(
            f"{self.http_way}historicalTrades?symbol={self.symbol}&fromId={fromId}&limit={limit}"
        ).json()
...
class Client:
    def __init__(
        self, api_key: str, sec_key: str, testnet: bool = False, symbol: str = "BTCUSDT"
    ):
        """
    ...
    def query_order(self, orderId):
        ...
        querystring = urllib.parse.urlencode(
            {"symbol": self.symbol, "orderId": orderId, "timestamp": self.timestamp()}
        )
    ...
    def change_leverage(self, leverage):
        ...
        req = "leverage?"
        querystring = urllib.parse.urlencode(
            {"symbol": self.symbol, "leverage": leverage, "timestamp": self.timestamp()}
        )
    ...
    def income_history(self, limit: int = 1000):
        ...
        req = "income?"
        querystring = urllib.parse.urlencode(
            {"symbol": self.symbol, "timestamp": self.timestamp(), "limit": limit}
        )
    ...
    def user_update_socket(self, on_message, on_error, on_close):
